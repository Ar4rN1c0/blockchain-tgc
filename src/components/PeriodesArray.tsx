        ...
        <section className={styles.timetable}>
            ...
            <div className={styles.cols} id={today}>          
                ...
                {frees[0].periodes.map((period: periodType, index: number) => (
                        period.isOc ? <Occupied></Occupied> : <div><Free periode={`${getPeriod(index)}`} date={`${today}`}>{`${getPeriod(index)}`}</Free></div>
                    ))}
            </div>
            <div className={styles.cols} id={tomorrow}>
                <label>{tomorrow}</label>
                {frees[1].periodes.map((period: periodType, index: number) => (
                        period.isOc ? <Occupied></Occupied> : <Free  periode={`${getPeriod(index)}`} date={`${tomorrow}`}>{`${getPeriod(index)}`}</Free>
                    ))}
            </div>
            <div className={styles.cols} id={next}>
                <label>{next}</label>
                {frees[2].periodes.map((period: periodType, index: number) => (
                        period.isOc ? <Occupied></Occupied> : <Free periode={`${getPeriod(index)}`} date={`${next}`}>{`${getPeriod(index)}`}</Free>
                    ))}
            </div>
            
            <div>
                {state ? <Send></Send> : 'error'}
            </div>
        </section>
    )
}